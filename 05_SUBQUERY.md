...
+ 
  ...
  + 서브쿼리를 사용하는 이유
    ...
    + **알려지지 않은 기준**이 먼지 잘 모르겠지만 정리하면서 생각해 보자..!!
    >
  ```SQL
  SUBQUERY 맛보기!!
...
### RANK() OVER / DENSE_RANK() OVER
  ...
  ```
  >
+ **알려지지 않은 기준** 이라는 것은 아직도 잘 이해가 되지 않는다. 
+ 추가적으로 알아봐야할 목록들
  + 스칼라 서브쿼리
  + 상관 서브쿼리
  + ROW_NUMBER() OVER
    + **ROWNUM**은 SELECT로 뽑아오는 테이블에 번호를 붙이는 것이고  
    **ROW_NUMBER() OVER** 같은 경우에는 SELECT로 테이블을 뽑아오고 원하는 데이터에 순서를 매길 수 있다.
  + **PARTITION BY**
    + 한 테이블 안에서 테이블을 **분할하는 기능**을 제공한다.
